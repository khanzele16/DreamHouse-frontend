...
export function CardItemPreview({ card }: CardItemPreviewProps) {
  ...
  return (
    <div
      className="font-[family-name:var(--font-stetica-bold)] w-full rounded-2xl shadow-md overflow-hidden hover:shadow-lg flex flex-col h-full"
      style={{
    ...
    >
      <div className="relative w-full aspect-[16/10] sm:aspect-[5/3]">
        ...
        />
        <button
          className="cursor-pointer absolute top-2 right-2 sm:top-3 sm:right-3 lg:top-4 lg:right-4 p-1.5 sm:p-2 rounded-full bg-black/30 hover:bg-black/50 transition-all duration-200 backdrop-blur-sm z-10"
      ...
      <div className="w-full flex flex-col gap-y-1 sm:gap-y-[5px] px-4 sm:px-5 lg:px-[25px] py-3 sm:py-4 lg:py-[15px] flex-grow">
        <div className="w-full flex flex-row justify-between items-start gap-x-2 sm:gap-x-3">
          <p
          ...
          <div className="flex flex-row items-center gap-x-1 sm:gap-x-[4px] flex-shrink-0">
            <svg
              className="w-[16px] h-[15px] sm:w-[18px] sm:h-[17px] lg:w-[23px] lg:h-[22px]"
              viewBox="0 0 23 22"
              fill="none"
              xmlns="http://www.w3.org/2000/svg"
            >
            ...
            <p className="text-lg sm:text-xl lg:text-[25px] text-[#F5BD68] leading-tight">
              {card.rating}
            </p>
          </div>
        </div>
        ...
        <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-2 sm:gap-x-2 mt-auto pt-2">
          ...
          <Link href={`/card/${card.id}`}>
            <button
              className="cursor-pointer py-1.5 sm:py-2 lg:py-2.5 px-3 sm:px-4 lg:px-5 rounded-2xl text-xs sm:text-sm lg:text-base whitespace-nowrap transition-colors flex-shrink-0"
              style={{
                color: "#fff",
